...
# 0 术语
本书中不同层的分组术语略有不同，且可以看做自定向下划分的。

整体上看，1.3.1.3提到，各种应用程序完成其任务需要交换报文(message)，源主机将报文划分成较小的数据块，并称之为分组（packet）。
分组通指运输层、网络层、链路层分组，运输层、网络层、链路层又各自有其术语。

3.1.2节开头提到，运输层分组称之为报文段（segment），网络层分组称之为数据报（data gram）。

4.1.1节提到了二、三分组交换机，再次佐证了分组（packet）是一个比较通用的概念。

5.1.1节开头提到，链路层交换的数据单元称之为帧（frame）。
```
...
在linux编程中，通常将UDP被称之为SOCK_DGRAM，而TCP被称之为SOCK_STREAM。但
SOCK_DGRAM与SOCK_STREAM的具体含义并不是UDP与TCP而是与地址族相关的。

# 3 运输层

https://blog.csdn.net/Cecilia3333/article/details/80358201
